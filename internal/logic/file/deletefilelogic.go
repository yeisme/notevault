...
func (l *DeleteFileLogic) DeleteFile(req *types.FileDeleteRequest) (resp *types.FileDeleteResponse, err error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, fmt.Errorf("file not found or already moved to trash")
		}
	...
	if file.UserID != userId {
		return nil, fmt.Errorf("no permission to move this file to trash")
	}
	...
	if err != nil {
		l.Error("file trash operation transaction failed", logx.Field("fileID", req.FileID), logx.Field("error", err))
		return nil, err
	}

	// Construct response
	resp = &types.FileDeleteResponse{
		Message: fmt.Sprintf("File moved to trash successfully (OSS file preserved): user=%s, fileID=%s, fileName=%s", userId, req.FileID, file.FileName),
	}

	l.Infof("File moved to trash successfully (OSS preserved): user=%s, fileID=%s, fileName=%s", userId, req.FileID, file.FileName)
